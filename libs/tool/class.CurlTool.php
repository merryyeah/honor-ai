...
class CurlTool {
	...
	public function __construct($setopt = array()) {
		...
		if( $this->setopt ['header'] ){
			$header = $this->setopt ['header'];
		}
		else{
			$header [] = 'Expect:';
		}
		...
		if ($this->setopt ['useCookie']) {
			...
			$cookfile = tempnam ( sys_get_temp_dir () . 'use_cookie/', 'cuk' );
			// 连接关闭以后，存放cookie信息
	...
	public function head($url, $params = array(), $referer = '') {
		return $this->_request ( 'HEAD', $url, $params, array (), $referer );
	}
	 ...
	 *        	:请求的URL
	 * @param array $params
	 *        	：请求的参数,格式如: array('id'=>10,'name'=>'yuanwei')
	 * @param array $referer
	 *        	:引用页面,为空时自动设置,如果服务器有对这个控制的话则一定要设置的.
	 ...
	 * @param array $referer
	 *        	:引用页面,引用页面,为空时自动设置,如果服务器有对这个控制的话则一定要设置的.
	 * @return 错误返回:false 正确返回:结果内容
	 */
	...
	/**
	 * 设置cookie值
	 * @param unknown_type $cookie_value
	 */
	public function setHeaderCookie($cookie_value){
	 ...
	 * @return 错误返回:false 正确返回:结果内容
	 */
	protected function _request($method, $url, $params = array(), $uploadFile = array(), $referer = '') {
		...
		if ($method == 'POST') {
			...
			if ( is_array($params)) {
				// 设置POST字段值
				$postData = $this->_parsmEncode ( $params, ($uploadFile?false:true) );
			} elseif (self::checkJson($params)) {
				$postData = $params;
			}
		...
		// 设置了引用页,否则自动设置
		if (empty($referer) && !empty($this->setopt['referer'])) {
			$referer = $this->setopt['referer'];
		}
		if ($referer) {
		...
		if ($this->errno () == 0) { // && $this->info['after']['http_code'] == 200
			...
			$this->info['after']['curl_error'] = curl_error($this->ch);
			return false;
		}
	}
